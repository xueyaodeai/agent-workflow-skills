...
Use the core contract for sources, authority, and observable completion; do not restate it as generic prompting advice. Keep the prompt model-independent. Specify reasoning effort only when configuring an API or evaluation and evidence justifies changing the existing baseline. Omit formatting or personality instructions that do not change behavior.
