...
When creating or revising a task plan or roadmap, connect each required outcome to its source, owning work, and observable acceptance condition. Define the scenario, expected behavior, verification method, and evidence to obtain; distinguish planned checks from observed results. Each milestone needs an outcome, necessary inputs, and exit checks; the final project criteria must also prove the overall flow or deliverable at the authorized delivery boundary. Resolve consequential ambiguity before planning dependent work, and label implementation assumptions separately from requirements. If `plan-outcomes` is available, use it for this clarification and decomposition; otherwise apply these rules directly. Keep the contract in the existing artifact rather than creating a second ledger.
...
- Require independent review when the user or an applicable rule requires it, or when the change materially affects a public contract, security or sensitive data, production or irreversible side effects, critical fail-closed behavior, a milestone exit, or a cross-repository integration gate. Coordination level, file count, duration, attempts, model cost, and bounded external reads alone do not trigger review.
