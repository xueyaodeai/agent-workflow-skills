...
- [ ] Final outcome and measurable completion criteria are aligned.
...
- Roadmap path: <path or `not generated`>
- Generation status: `not_ready | draft_unreviewed | reconciled`
- Carryover mapping verified: <yes/no/not applicable>
- Decisions omitted from roadmap: <none or explicit list and reason>
- First milestone and entry criteria: <milestone/criteria>
- Reconciliation evidence: <review/audit/path>

## External cross-model review

- Status: `not_requested | unavailable | completed_inconclusive | findings_open | passed`
- Availability: `available_in_executor | available_host_only | unavailable | not_checked`
- Availability probe: <binary/auth/model/read-only/input-authorization result, CLI version/date/source, or `not run`>
- Outside-sandbox authorization: `not_required | pending | granted | declined | unavailable`
- Authorization scope: <exact model, snapshot path/hashes, external-transfer/cost disclosure, and one-shot command identity, or `none`>
- Provider/model: <for example `Cursor/claude-opus-5-thinking-medium` or `not available`>
- Session/review identity: <session ID and reviewed artifact hashes, or `none`>
- Verdict/findings: <GO/CONDITIONAL_GO/NO_GO and report path, or bounded unavailability reason>
- Targeted re-review: <session ID/verdict or `not required`>
